...
class TGN(torch.nn.Module):
  ...
  def compute_temporal_embeddings(self, source_nodes, destination_nodes, negative_nodes, edge_times,
    ...
    NUM_NEG_TRAIN = int(negative_nodes.shape[0]/source_nodes.shape[0])
    ...
    # n_samples = len(source_nodes) 
    # # print('source_nodes, ',source_nodes.shape, 'destination_nodes, ', destination_nodes.shape, 'negative_nodes, ', negative_nodes.shape)
    # nodes = np.concatenate([source_nodes, destination_nodes, negative_nodes])
    # positives = np.concatenate([source_nodes, destination_nodes])
    ...
    if self.use_memory:
      ...
      p_neg_time_diffs = torch.LongTensor(edge_times_duplicated).to(self.device) - last_update[negative_nodes].long() 
      ...
      # print(torch.LongTensor(edge_times).to(self.device).shape, last_update[negative_nodes].long().shape)
    ...
    """
    2. Embedding (Graph Attention)
    """
    
    # Compute the embeddings using the embedding module # shape: torch.Size([3*bs, 172]) # source, destination, negative 각각 100개씩
    ...
    if self.use_memory:
      if self.memory_update_at_start:
        ...
        # print('@@@@@@@check memeory@@@@@@@@@')
        # print(memory[positives][0])
        # print(self.memory.get_memory(positives)[0])
  ...
  def compute_temporal_embeddings_eval(self, source_nodes, destination_nodes, p_neg_nodes, 
                                        edge_times, edge_idxs, n_neighbors=10):
    """
    Compute temporal embeddings for sources, destinations, and negatively sampled destinations.
    ...
    # 다음 배치로 넘어가더라도 TGN.memory에는 이전 배치의 메모리가 남아 있음
    memory = None
    time_diffs = None
    if self.use_memory:
      if self.memory_update_at_start:
        ...
        memory, last_update = self.get_updated_memory(list(range(self.n_nodes)),
                                                      self.memory.messages) # dict, 키는 노드id, 밸류는 메세지
      ...
      try:
          ...
          print("edge_times_duplicated shape:", torch.LongTensor(edge_times_duplicated).shape)
          print("last_update shape:", last_update.shape)
          print("p_neg_nodes max index:", p_neg_nodes.max().item())
          print("last_update device:", last_update.device)
          print("self.device:", self.device)
          print("last_update[p_neg_nodes] shape:", last_update[p_neg_nodes].shape)
          print("edge_times_duplicated device:", torch.LongTensor(edge_times_duplicated).to(self.device).device)
          # Add any additional debug information you think might be relevant
          raise
    ...
    # shape: torch.Size([3*bs, dim]) # source, destination, negative 각각 bs개씩
    node_embedding = self.embedding_module.compute_embedding(memory=memory,
                                                             source_nodes=nodes,
                                                             timestamps=timestamps,
                                                             n_layers=self.n_layers,
  ...
  def compute_temporal_embeddings_eval_jodie(self, source_nodes, destination_nodes, p_neg_nodes, 
    ...
    nodes = np.concatenate([source_nodes, destination_nodes, p_neg_nodes])
    positives = np.concatenate([source_nodes, destination_nodes])
    edge_times_duplicated = [x for x in edge_times for _ in range(10)]
    timestamps = np.concatenate([edge_times, edge_times, edge_times_duplicated])
